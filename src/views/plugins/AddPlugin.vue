<template>
  <div>
    <Breadcrumb>
      ...
      <BreadcrumbItem to="/plugins">{{ $t('breadcrumb.plugin') }}</BreadcrumbItem>
      <BreadcrumbItem v-if="!pluginId">{{ $t('breadcrumb.addPlugin') }}</BreadcrumbItem>
    ...
    <Form :model="formItem" :label-width="140" style="margin-top: 20px">
      ...
      <FormItem>
        <Button type="primary" @click="savePlugin">{{ $t('common.save') }}</Button>
      </FormItem>
    </Form>
  </div>
</template>
