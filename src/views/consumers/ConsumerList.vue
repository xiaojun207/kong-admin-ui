<template>
  <div id="content">
    <Breadcrumb>
      ...
      <BreadcrumbItem to="/consumers">{{ $t('breadcrumb.consumer') }}</BreadcrumbItem>
      <BreadcrumbItem>{{ $t('breadcrumb.consumer_list') }}</BreadcrumbItem>
    </Breadcrumb>
    <Row>
      <Col span="12"><h1 style="width: 100px;height: 40px"></h1></Col>
      <Col span="12" style="text-align:right;position: absolute;top: 30%;right: 0px">
        <Button type="primary" size="small" @click="addConsumer">{{ $t('breadcrumb.addConsumer') }}</Button>
      </Col>
    </Row>
